...
def tool_metrics(records: list[dict[str, Any]]) -> dict[str, float]:
    ...
    for row in records:
        ...
        # A malformed response is not a successful tool decision even if a
        # JSON fragment can be recovered from it.
        pred_needs = valid and bool(pred) and pred.get("tool_name") != "none"
        if gold_needs and pred_needs:
            ...
            tn += 1
        tool_correct += int(valid and _same_tool(pred, gold))
        args_correct += int(valid and _arguments_match(pred, gold))
    n = max(len(records), 1)
