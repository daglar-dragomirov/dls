...
*{box-sizing:border-box}body{margin:0;font-family:Inter,system-ui,sans-serif;color:var(--ink);background:var(--bg);letter-spacing:0}.topic{display:inline-block;margin-bottom:10px;padding:5px 8px;background:#fff;color:#17212b;border-left:4px solid #f59e0b;font-size:14px}header{padding:25px max(20px,calc((100% - 1080px)/2));background:#17212b;color:#fff;border-bottom:4px solid #f59e0b}h1{margin:0 0 6px;font-size:27px}header p{margin:0;color:#cbd5e1}
...
textarea,input,select{width:100%;padding:10px;border:1px solid #bbc6d1;border-radius:5px;font:inherit;margin:6px 0}textarea{min-height:92px;resize:vertical}button,.download{display:inline-block;border:0;border-radius:5px;padding:10px 14px;margin:8px 7px 0 0;background:var(--accent);color:#fff;font-weight:700;cursor:pointer;text-decoration:none}button:disabled{cursor:wait;opacity:.65}.download{background:#e7edf0;color:#263842}pre{min-height:390px;max-height:620px;margin:0;background:#101820;color:#d8f3e8;padding:15px;overflow:auto;border-radius:6px;white-space:pre-wrap;font-size:13px}.status{font-size:13px;color:var(--muted);margin-top:8px}@media(max-width:800px){main{grid-template-columns:1fr}.wide{grid-column:auto}.metrics{grid-template-columns:1fr 1fr}}
</style></head><body><header><div class="topic"><strong>Выбранная тема DLS: «Speech-Toolformer»</strong></div><h1>Голосовой ассистент Speech-Toolformer</h1><p>Аудиозапросы, JSON-вызовы функций и сравнение прямого и каскадного пайплайнов</p></header>
...
<label for="text">Текстовый запрос</label><textarea id="text">Split a USD 120 bill between 4 people with 15% tip</textarea><button id="textButton" onclick="runText()">Выполнить текстовый запрос</button>
...
<select id="pipeline"><option value="direct">Прямой: аудио → вызов функции</option><option value="cascaded">Каскадный: ASR → текстовая модель → вызов функции</option></select><button id="audioButton" onclick="runAudio()">Обработать аудио</button><div class="status" id="status">Готово к проверке.</div></section>
<section class="panel" id="resultPanel"><h2>Результат и трасса пайплайна</h2><pre id="out">Введите текстовый запрос или выберите аудиофайл, затем запустите обработку.</pre></section></main>
<script>
function showResult(payload){document.getElementById('out').textContent=JSON.stringify(payload,null,2);if(window.innerWidth<=800)document.getElementById('resultPanel').scrollIntoView({behavior:'smooth',block:'start'});}
async function runText(){const status=document.getElementById('status'),text=document.getElementById('text'),button=document.getElementById('textButton');button.disabled=true;status.textContent='Текстовый запрос обрабатывается…';try{const response=await fetch('/assistant/text',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:text.value})});const payload=await response.json();showResult(payload);const tool=payload.call?.tool_name||'нет инструмента';status.textContent=response.ok?`Готово: ${tool}`:`Ошибка: ${payload.detail||'запрос не выполнен'}`;}catch(error){showResult({error:String(error)});status.textContent='Ошибка сети';}finally{button.disabled=false;}}
async function runAudio(){const status=document.getElementById('status'),audio=document.getElementById('audio'),pipeline=document.getElementById('pipeline'),button=document.getElementById('audioButton');const file=audio.files[0];if(!file){showResult({error:'Сначала выберите аудиофайл.'});status.textContent='Аудиофайл не выбран';return}button.disabled=true;status.textContent='Аудио обрабатывается…';try{const form=new FormData();form.append('audio',file);form.append('pipeline',pipeline.value);const response=await fetch('/assistant/audio',{method:'POST',body:form});const payload=await response.json();showResult(payload);const tool=payload.call?.tool_name||'нет инструмента';status.textContent=response.ok?`Готово: ${tool}`:`Ошибка: ${payload.detail||'обработка не выполнена'}`;}catch(error){showResult({error:String(error)});status.textContent='Ошибка сети';}finally{button.disabled=false;}}
fetch('/metrics').then(r=>r.json()).then(m=>{const t=m.text_pipeline||{},l=m.local_lfm||m.real_audio||{},c=l.pipeline_C_direct_audio||l.pipeline_C_native_audio||{},d=l.pipeline_D_cascaded||{},b=l.pipeline_B_asr||{};const values=[['F1 на тексте',t.f1],['F1 прямого пайплайна',c.f1],['Аргументы C',c.argument_accuracy],['Аргументы D',d.argument_accuracy],['WER локальной ASR',b.wer]];document.getElementById('metrics').innerHTML=values.map(([k,v])=>`<div class="metric"><strong>${Number(v||0).toFixed(3)}</strong><span>${k}</span></div>`).join('')}).catch(()=>{document.getElementById('metrics').textContent='Метрики временно недоступны.'});
</script></body></html>"""
