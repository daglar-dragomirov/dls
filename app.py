...
button{margin-top:13px;border:0;border-radius:5px;padding:10px 14px;font-weight:700;cursor:pointer;background:var(--accent);color:#fff}button.secondary{background:#e7edf0;color:#20323a;margin-left:6px}pre{margin:0;min-height:460px;max-height:650px;overflow:auto;background:#111b20;color:#d9f2e8;border-radius:6px;padding:14px;white-space:pre-wrap;font-size:13px}
...
<button onclick="runAgent()">Оценить</button><button class="secondary" onclick="loadNegative()">Сложный пример</button><div class="status" id="status"></div></section>
<section class="panel"><h2>Решение и трасса инструментов</h2><pre id="output">Загрузка…</pre></section></main>
<script>
const ids=['query','organization_name','category','address','prices_summarized','review_snippets'];
async function runAgent(){const status=document.getElementById('status');status.textContent='Агент анализирует карточку…';const body={};ids.forEach(id=>body[id]=document.getElementById(id).value);body.use_web_search=document.getElementById('use_web_search').checked;const response=await fetch('/score',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const data=await response.json();document.getElementById('output').textContent=JSON.stringify(data,null,2);status.textContent=response.ok?'Готово':'Ошибка запроса';}
